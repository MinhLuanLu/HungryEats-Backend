...
async function orderHandler(order, socket, io) {
    try{
        ...
        const save_order_status = await Make_Query(`INSERT INTO Order_status (User_id, Order_id) VALUES (${get_sender_id?.User_id}, ${order_id})`)
        // send order back to buyer (sender)
        socket.emit('sending_order_status', get_Order)
