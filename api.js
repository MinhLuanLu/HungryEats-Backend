...
Socket_Connection(api_express) //// Run the SocketIO as Port:3001
...
api_express.post('/order_status/api', async (request, response) => {
    ...
    const [get_user_id] = await Make_Query(`SELECT User_id FROM Users WHERE Email = '${Email}'`)
    let User_id = get_user_id["User_id"]
    const get_order_id = await Make_Query(`SELECT * FROM Order_status WHERE User_id = ${User_id}`)
   
    let Order_status_list = []
    for (let i = 0; i< get_order_id.length; i++){
        let order_list_id = get_order_id[i]["Order_id"]
        const [get_order_list] = await Make_Query(`SELECT * FROM Orders INNER JOIN Food ON Orders.Food_id = Food.Food_id WHERE Order_id = ${order_list_id}`)
        Order_status_list.push(get_order_list)
    }
    //console.log(Order_status_list)
    response.status(200).send({
...
api_express.post('/order_history/api', async(request, response)=>{
    ...
    const [get_user_id] = await Make_Query(`SELECT User_id FROM Users WHERE Email = '${Email}'`)
    let User_id = get_user_id["User_id"]
    const get_order_history = await Make_Query(`SELECT * FROM Orders INNER JOIN Food ON Orders.Food_id = Food.Food_id WHERE User_id = ${User_id}`);

    console.info('Order history retrieved successfully!')
